...
createInboxNotification = function (url, type, recipient) {
  var user = Meteor.user();
  notificationExists = Inbox.findOne({url: url});
...
Meteor.methods({
  ...
  sendmessagetest: function(messageTestAttributes) {
    ...
    var messageExists = Messages.findOne({recieving: {$in: [userExists._id, this.userId]}});
    if (messageExists) {
      ...
      var createMessage = Messages.insert(message);
      return createMessage;
    }
  },
  sendtext: function(textAttributes) {
    var user = Meteor.user();
        parties = Messages.findOne({_id: textAttributes.messageId});
        console.log(parties.recieving);
  ...
  favorite: function(favorite, userId) {
    ...
    var affleckted = Meteor.users.update({ 
      _id: this.userId },
      {
        $addToSet: {favorited: favorite.postId}
    });
  ...
  updatePicture: function(pic) {
    Meteor.users.update({_id: this.userId}, {$set: {profile: pic}});
  }
});